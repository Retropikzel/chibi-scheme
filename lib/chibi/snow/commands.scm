        ...
        (let* ((current-version
          ...
          (cond
           ((member (car ls) ignored)
            (lp (cdr ls) res ignored))
           ((and (null? candidates)
                 (assoc (car ls) current))
            (when (and (not (equal? (car (car ls)) 'scheme))
                       (or (equal? impl 'chibi)
                           (and (member (car ls) lib-names)
                                (not (equal? (car (car ls)) 'scheme))
                                (not (implementation-supports-natively? impl cfg (car ls))))))
           ...
           ((and (not (assoc (car ls) current))
            ...
            (when (and (pair? (car ls))
                       (not (equal? (car (car ls)) 'scheme)))
              (info "Skipping natively implemented library: " (car ls)))
            (lp (cdr ls) res (cons (car ls) ignored)))
           ((or (null? candidates)
                (and (null? candidates) (member (car ls) lib-names)))
            (cond
             ((or (equal? (car (car ls)) 'scheme)
