    ...
    (let lp ((ls candidates) (i 1))
      (if (pair? ls)
          (let ((pkg (car ls)))
            ...
            (display " ") (display (package-publisher '() pkg))
...
(define (expand-package-dependencies repo impl cfg lib-specs)
  (let ((current (installed-libraries impl cfg))
        (auto-upgrade-dependencies?
         (conf-get cfg '(command install auto-upgrade-dependencies?))))
    (let lp ((ls lib-specs) (res '()) (ignored '()))
      (cond
       ((null? ls) res)
       ((find (lambda (pkg)
                (package-provides? pkg (assoc-get (car ls) 'name))) res)
        (lp (cdr ls) res ignored))
       (else
        (let* ((lib-name (assoc-get (car ls) 'name))
               (current-version
                (cond ((assoc lib-name current)
                       => (lambda (x) (package-version (cdr x))))
                      (else #f)))
               (providers
                (filter (lambda (pkg) (package-provides? pkg lib-name))
                        (cdr repo)))
               (candidates
                (filter
                 (lambda (pkg)
                   (or (not current-version)
                       (and (or auto-upgrade-dependencies?
                                (member lib-name lib-specs))
                            (version>? (package-version pkg)
                                       current-version))))
                 providers)))
          (cond
           ((member lib-name ignored)
            (lp (cdr ls) res ignored))
           ((and (null? candidates)
                 (assoc lib-name current))
            (when (and (not (equal? (car lib-name) 'scheme))
                       (or (equal? impl 'chibi)
                           (and (member lib-name lib-specs)
                                (not (equal? (car lib-name) 'scheme))
                                (not (implementation-supports-natively? impl cfg lib-name)))))
                (warn "skipping already installed library: " lib-name))
            (lp (cdr ls) res (cons lib-name ignored)))
           ((and (not (assoc lib-name current))
                 (pair? lib-name)
                 (implementation-supports-natively? impl cfg lib-name)
                 (not (conf-get cfg '(command install overwrite-native?))))
            ;; assume certain core libraries already installed
            (when (and (pair? lib-name)
                       (not (equal? (car lib-name) 'scheme)))
              (info "Skipping natively implemented library: " lib-name))
            (lp (cdr ls) res (cons lib-name ignored)))
           ((or (null? candidates)
                (and (null? candidates) (member lib-name lib-specs)))
            (cond
             ((or (equal? (car lib-name) 'scheme)
                  (yes-or-no? cfg "Can't find package: " lib-name
                              ".  Proceed anyway?"))
              (lp (cdr ls) res (cons lib-name ignored)))
             (else
              (die 2 "No candidates, not installing: " lib-name))))
...
(define-memoized (library-in-repository? repo lib-spec)
  (call-with-current-continuation
    (lambda (return)
      (for-each (lambda (pkg)
                  (when (package-provides? pkg (assoc-get lib-spec 'name))
                    (return #t)))
                (cdr repo))
      #f)))
  ...
  (let*-values
      (((repo) (current-repositories cfg))
       ...
       ((impl-cfgs) (map (lambda (impl)
                           (conf-for-implementation cfg impl))
                         impls))
       ((package-files lib-specs) (partition package-file? args))
       ((lib-specs)
        (let ((lib-specs (map parse-library-spec lib-specs)))
          (when (not (pair? package-files))
            (for-each
              (lambda (lib-spec)
                (when (not (library-in-repository? repo lib-spec))
                  (die 2 "Can't find package: " lib-spec)))
              lib-specs))
          lib-specs))
       ((start-dependencies)
        (lambda (impl cfg)
